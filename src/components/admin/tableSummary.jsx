...
const TableSummaryComponent = () => {
  ...
  const fetchSummary = () => {
    ...
    .then((res) => {
      if (res.success) {
        setDataSummary(res.data.summaries)
      }
    })
    .catch((err) => {
      if (err.status === 401) {
  ...
  const handleSaveAsXLSX = () => {
    const worksheet = XLSX.utils.json_to_sheet(dataSummary);
  ...
  return (
    <>
      <form className="flex justify-between items-center flex-wrap mb-6 text-xs text-gray-700">
      ...
      <div className="float-right">
        <button
          className={`${
            dataSummary.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#198754] hover:bg-[#218838] cursor-pointer'
      ...
      >
        <Box className="modal absolute overflow-hidden overflow-y-scroll bg-white top-1/2 left-1/2 h-[90%]">
          ...
          <div>
            <div className="bg-[#f8f9fa] p-4 mb-5 m-4 rounded-lg" id="summaryMeta">
              ...
              <div className="flex gap-x-5 items-center mb-2">
                ...
                <div className="text-[#212529]">{detailSummary.user_username}</div>
              </div>
              <div className="flex gap-x-5 items-center mb-2">
